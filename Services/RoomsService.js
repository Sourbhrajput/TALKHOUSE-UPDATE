...
class RoomsService {
   ...
   async getRooms({ find, search, _id }) {
      ...
      return await RoomsModel.find({ roomtype: { $in: find }, ownerId: _id }).populate("speakers").populate("ownerId").exec();
   }
   ...
   async deleteRoomById({ roomId }) {
      ...
      return await RoomsModel.deleteOne({ _id: roomId });
