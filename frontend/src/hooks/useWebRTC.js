...
export const useWebRTC = (roomId, user, setClientFeature, setRoom, setShowHand,setOpenDC,setAllMessage) => {
    ...
    const init = async () => {
      ...
      await addMedia().then(() => {
        addNewClient({ ...user, mute: muteState, ishandRaise: ishandRaiseState }, () => {
          ...
          socket.current.on(ACTION.ROOMDELETED,roomDeleted);
      ...
      const handelNewPeer = async ({ peerId, user: remoteUser, createOffer }) => {
        ...
        userPeer.ondatachannel=(e)=>
        {
         ...
         { 
          setAllMessage((m)=>
          { 
            let allMssg=JSON.parse(JSON.stringify(m));
            const messageRec= JSON.parse(e.data);
            messageRec.type='incomming';
            allMssg.push(messageRec)
            return allMssg;
          })
         }
        }
         ...
         openDC.push(userPeer.createDataChannel('dataChannel'));
        //  handle on track 
